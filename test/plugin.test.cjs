...
test("plugin registers standalone weather service and retracts it on stop",async(t)=>{
	...
	const messages=[]; const app={getDataDirPath:()=>directory,setPluginStatus(){},handleMessage(_id,message){messages.push(message);},subscriptionmanager:{subscribe(){}},ajrmMarineLocations:{contract:"ajrm-marine-locations-service-v1",async list(){return [forecastLocation];},async get(id){return id===forecastLocation.id?forecastLocation:null;}}};
	...
	assert.equal(status.weatherLocationCount,1); assert.equal(status.locationsService,"ajrm-marine-locations-service-v1");
	assert.deepEqual(await app.ajrmMarineWeatherDatabase.listLocations(),[{id:"weather-1",name:"Cuan Sound weather forecast",description:"Forecast point",position:{longitude:-5.63,latitude:56.27},revision:3}]);
